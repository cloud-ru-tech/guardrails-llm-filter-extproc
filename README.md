...
**Маскирование PII и секретов в трафике к LLM в виде Envoy external processor.**
...
- **Поддерживаемые API**: OpenAI `/v1/chat/completions`, `/v1/responses`, Anthropic
  ...
  `GUARDRAILS_PATHS`. Legacy OpenAI `/v1/completions` (prompt-стиль) **не поддерживается**
  — не мапьте его в `GUARDRAILS_PATHS`; запрос, чьё тело формат не может разобрать,
  проходит без маскирования (fail-open) и увеличивает
  `extproc_guardrails_unsupported_body_schema_total`.
- **Fail-open по замыслу**: любая внутренняя ошибка пропускает трафик, а не ломает его
  (в связке с Envoy `failure_mode_allow` вы выбираете общую позицию).
...
Пример — добавить кастомное правило детекции (валидируется тем же путём компиляции, что и
в проде, применяется атомарно без рестарта, сохраняется в хранилище):
...
Apache-2.0 — см. [LICENSE](LICENSE). Включает правила, производные от gitleaks (MIT), см.
[NOTICE](NOTICE).
